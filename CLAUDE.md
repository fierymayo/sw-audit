...
6. **matcher.py is a verbatim port of the Mechanic Liquid matcher.** Parity is the whole point — reason codes are only facts because the audit predicts exactly what the task will do. Never "improve" it. The pipeline is:
...
7. **Reason codes are facts, not guesses:**
   ...
   - `SYNC_MISS` — task matcher hits on an older product; task should have filled it; investigate
...
10. **The 29-list is the single Job-3 reference.** Theme-side PRIORITY comparison was explicitly descoped by the operator.
