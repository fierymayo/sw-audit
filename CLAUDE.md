...
16. **subcat_sim.py is a parity port like matcher.py** — of the SubCat task's classification
    `case` tree. Substring `in` semantics (NOT word-boundary), apostrophes removed without
    a space in title_words, whitespace NOT collapsed, and the pre-wrap asymmetry
    (Sock-Tape only under Misc, never Accessories) is a live task bug the simulator exists
    to surface — never "fix" it in the port. classification_type mapping and the eligible
